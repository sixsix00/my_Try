...
//			int value = (b & 0x7F) + (b < 0 ? 128 : 0);
	...
	public static String getFileName(final Part part) {
		for (String content : part.getHeader("content-disposition").split(";")) {//Content-Disposiiotn: 霈�汗��隞交������蒂銝�����迂
			if (content.trim().startsWith("filename")) {//����隞卯ilename撘�憭�
	...
	public static void exploreParts(Collection<Part> parts, HttpServletRequest req) {
		try {
			...
			for (Part part : parts) {
				...
				String contentType = part.getContentType();//取得 Part 物件的 Content-Type 表頭內容。如果是一般的欄位或是沒有上傳資料的話會得到 null。
				String value = "";
